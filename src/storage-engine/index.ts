...
function storageEngine(db=required("db")): IStorageEngine{
    return {
        ...
        find: function({ model=required("model"), query=required("query"),sort={}  }): Promise<Array<any>>{
            return db.find(model, query, sort)
        },
        findOne: function(query =required("query")):any{
            return db.findOne(query)
        },
        update: function({ query =required("query"), model=required("model"), updateObj=required("updateObj") }){
            return db.update(model, query, updateObj)
        },
        removeOne: function({query=required("query"), model=required("model") }){
